...
namespace Shadowsocks.Proxy
{
    class ProxyAuthHandler
    {
        ...
        {
            Server GetCurrentServer(int localPort, string targetURI, bool cfgRandom, bool usingRandom, bool forceRandom)
            => _config.GetCurrentServer(localPort, targetURI, cfgRandom, usingRandom, forceRandom);
            ...
            if (_config.PortMapCache.ContainsKey(local_port))
            {
                ...
                if (cfg.server == null || cfg.id == cfg.server.Id)
                {
                    if (cfg.type == PortMapType.Forward) // tunnel
                    {
                        ...
                        && new Socks5Forwarder(_config).Handle(_remoteHeaderSendBuffer, _remoteHeaderSendBuffer.Length, _connection, local_sendback_protocol))
                    {
                    }
                    else
                    {
