...
internal class ShadowsocksFmt : BaseFmt
{
    ...
    private static readonly Regex DetailsParser =
        new(
    ...
    private static ProfileItem? ResolveSSLegacy(string result)
    {
        ...
        if (!Misc.IsNullOrEmpty(tag))
        {
            item.remarks = Misc.UrlDecode(tag);
        }
        Match details;
        try
        {
            details = DetailsParser.Match(Misc.Base64Decode(base64));
        }
        ...
        item.port = Misc.ToInt(details.Groups["port"].Value);
        return item;
    }
    ...
    private static ProfileItem? ResolveSip002(string result)
    {
        ...
        if (rawUserInfo.Contains(':'))
        {
            ...
            item.id = Misc.UrlDecode(userInfoParts[1]);
        }
        else
        {
        ...
        var queryParameters = Misc.ParseQueryString(parsedUrl.Query);
    ...
    {
        //SsSIP008
        var lstSsServer = Json.FromJson<List<SsServer>>(result);
        if (lstSsServer?.Count <= 0)
        {
            var ssSIP008 = Json.FromJson<SsSIP008>(result);
        ...
        if (lstSsServer?.Count > 0)
        {
            ...
            foreach (var it in lstSsServer)
            {
                var ssItem = new ProfileItem()
                {
                    ...
                    id = it.password,
                    address = it.server,
                    port = Misc.ToInt(it.server_port)
                };
                lst.Add(ssItem);
            }
            return lst;
